---
type: schema_base
tags:
  - L1
  - schéma-base
  - compétence
  - inner-mapping
besoin_SDT: "Compétence"
domaines_Young:
  - "Responsabilité et Standards excessifs"
  - "Autonomie et Performance altérées"
date: 2026-06-06
version: 2.0
---

# 🪨 L1 — Besoin de Compétence

> **Mode de lecture :**
> Section I : théorie et méthode — neutre, transposable (SDT · Young · Agon · LFB).
> Sections II–III : identification et instances personnelles — document vivant.
> Sections IV–VI : dynamiques, effets et travail en cours.

---

## I. Concept théorique

### I.1 Le besoin fondamental (Théorie de l'auto-détermination)

> [!abstract] ✅ Fondement solide — Deci & Ryan (2000)
> Le **besoin de compétence** est défini comme le sentiment d'être efficace dans ses actions — de pouvoir produire les effets souhaités sur son environnement. Universel, inné.
>
> Satisfait → motivation intrinsèque, sentiment d'efficacité, bien-être.
> Entravé chroniquement → doute sur ses capacités, diminution de la motivation, formation de schémas cognitifs défensifs.[^1]

### I.2 Quand le besoin est entravé — Structure cognitive (Young)

> [!abstract] ✅ Fondement solide — Young et al. (2003)
> Les **schémas maladaptatifs précoces** sont des prémisses profondes vécues comme des vérités, formées quand un besoin fondamental est chroniquement entravé. Validés sur 1054 adultes. Cohérence interne : .83–.96.[^2]
>
> **Limite critique :** validés en population clinique. Application au contexte de performance = extrapolation cohérente, non testée directement.

Deux domaines pertinents :

**Responsabilité et Standards excessifs** *(Excessive Responsibility & Standards)*
- *Exigences irréalistes* : standards très élevés pour éviter la honte — perfectionnisme, rigidité face à l'erreur
- *Punition envers soi* : erreurs méritent une punition sévère — pas de droit à l'erreur, absence de récupération

**Autonomie et Performance altérées** *(Impaired Autonomy & Performance)*
- *Estime de soi contingente à la performance* *(Contingent Self-Worth)* (Crocker & Park, 2004) : valeur personnelle indexée sur les résultats externes — monte et descend avec la variance[^3]

> [!warning] ⚠️ Fondement à nuancer — Young
> Ces schémas sont solides cliniquement. Application spécifique au poker : [inféré], mécanisme cohérent, non testé dans ce contexte.

---

### I.3 Mode d'expression Agon

> [!abstract] ✅ Fondement : Caillois (1958) · Blascovich & Tomaka (1996)
> Le Besoin de Compétence peut s'exprimer dans plusieurs modes. Le mode **Agon** est le mode compétitif pur : la compétence ne se *sent* que lorsqu'elle est testée contre une résistance réelle et incertaine.
> → Fondement complet : [[01 — Fondements théoriques/10 — Agon · Alea · Mimicry · Ilinx]]

**Caractéristique centrale :** pour les profils à dominante Agon, une compétence non testée est une compétence non confirmée. La victoire facile n'est pas satisfaisante — elle ne *prouve* rien. Le Besoin de Compétence n'est pleinement satisfait que lorsque la difficulté de l'adversaire correspond au niveau de ressources mobilisées.

**Interaction avec la variance au poker :**
La variance remplit la fonction d'adversaire Agon — aveugle, non manipulable, indifférente à l'identité. C'est paradoxalement la forme d'adversité la plus pure disponible dans un environnement probabiliste. Ce qui explique partiellement l'engagement accru dans les sessions défavorables : l'adversaire (la variance) y est plus résistant.

**Risque spécifique au mode Agon :**
Si l'adversité externe est nécessaire pour que le Besoin de Compétence soit satisfait, les sessions gagnantes "faciles" restent cognitivement et identitairement creuses — indépendamment de leur qualité stratégique réelle. La compétence n'a pas été *testée*, donc elle n'a pas été *sentie*.

**Piste de recalibrage :**
Construire une adversité interne — le standard de précision du processus comme adversaire. *"Est-ce que j'ai joué ce spot à la limite de ce que je sais faire ?"* plutôt que *"est-ce que j'ai gagné ?"*. Déplace le référentiel de l'externe (résultats) vers l'interne (processus), tout en préservant la fonction activatrice de l'Agon.


---


### I.4 Reformulation LFB — Le schéma comme prédiction entraînée

> [!todo] 🔬 Reformulation — Barrett (2017), intégrée dans le modèle Inner Mapping
> LFB reframe ce qu'est un schéma L1 : ce n'est pas une croyance stockée quelque part. C'est une **prédiction entraînée** — le cerveau defaulte dessus parce qu'il l'a pratiquée le plus souvent. Quand une prédiction est entraînée depuis longtemps, elle est aussi la plus économique métaboliquement.[^4]

**Ce que ça change dans la lecture des instances :**

Un schéma conscientisé n'est pas supprimé. Il n'est plus la prédiction *par défaut* — mais il reste disponible comme prédiction alternative. Le cerveau peut y revenir sous une condition précise : l'**épuisement du budget corporel** (*body budget*, terme technique : *allostase*).

> [!abstract] ✅ Fondement solide — Barrett (2017) + Deci & Ryan (2000)
> Quand le budget corporel est épuisé — mauvais sommeil, chaleur intense, stress prolongé, charge émotionnelle élevée — le cerveau a moins de ressources métaboliques pour des prédictions nuancées. Il revient aux patterns les plus économiques : les prédictions les plus entraînées, c'est-à-dire les schémas défensifs historiques.
>
> Ce n'est pas une rechute. C'est la biologie normale d'un cerveau sous-ressourcé.

**Ce que LFB dit du travail de conscientisation :**

*"You are continuously cultivating your past as a means of controlling your future."*

Chaque dépolarisation accomplie est de la culture de passé. Elle construit une nouvelle prédiction concurrente qui, avec la pratique, devient elle aussi économique. Le travail Inner Mapping est littéralement ça : remplacer progressivement les prédictions entraînées défensives par des prédictions entraînées génératives.

---

## II. Comment identifier son instance

*Section transposable à toute personne.*

### II.1 Questions diagnostiques

- Dans quels contextes l'*erreur* déclenche-t-elle une réaction plus forte que sa gravité objective ?
- Comment ma *confiance* varie-t-elle selon mes résultats récents — sur quelle fenêtre temporelle ?
- Est-ce que je *distingue* "j'ai mal fait ce coup" de "je suis insuffisant" ?
- Quand est-ce que l'*étude* ou la correction devient compulsive plutôt que fonctionnelle ?

### II.2 Signaux d'activation

- Réaction émotionnelle disproportionnée à la gravité réelle de l'erreur
- Étude acharnée ou review compulsive post-erreur (L4)
- Confiance qui monte avec un bon run, doute qui monte avec un mauvais run
- Difficulté à valider un play créatif sans justification mathématique

### II.3 Méthode de documentation

```
[pattern:: Perfectionnisme protecteur]
[pattern:: Intransigeance envers soi]
[pattern:: Résultats-dépendance]
```

---

## III. Mes instances — état actuel

*Première personne. Document vivant — à mettre à jour au fil du travail.*

---

### Perfectionnisme protecteur
**[documenté — Alexis 04-20-2026]** · → [[L2 - Schémas Cognitifs/12 — Orientation vers l'idéal -- Perfectionnisme|L2 Perfectionnisme / Ambition]]

**Prédiction centrale :**
*"La maîtrise technique est un rempart identitaire — si la théorie est appliquée à 100%, l'estime de soi est protégée."*

**État de conscientisation :**
🟠 Prédiction identifiée — dépolarisation en cours

**Conditions de résurgence (budget corporel) :**
- Haute énergie perçue → attentes qui explosent → peur de gâcher la forme
- Anticipation d'une session "importante"
- Contexte d'évaluation par des pairs compétents

**Dépolarisations accomplies :**
- [[Mini-Dépolarisation - Haute Énergie et performeur idéal]] — travail direct sur la pression du performeur idéal
- [[mini-dépo - être sur-motivé vs ne pas l'être]] — exploration des coûts de la sur-motivation

**Ce qui ressurgit encore :**
L'enjeu perçu qui explose en haute énergie (Piédestal). L'étude acharnée post-erreur sur des spots connus.

**Ce qui a changé :**
Je peux nommer le pattern en session. Je reconnais la contraction qui vient avec la haute énergie avant qu'elle ne dégrade la performance.

---

### Intransigeance envers soi
**[documenté — Alexis 04-07-2026]**

**Prédiction centrale :**
*"L'erreur sur ce que je devrais maîtriser est impardonnable."*

**État de conscientisation :**
🟢 Prédiction dépolarisée — ressurgit sous conditions de budget épuisé

**Conditions de résurgence (budget corporel) :**
- Fatigue + erreur technique sur spot connu = combinaison déclenchante
- Fin de longue session
- Mauvaise nuit précédant la session

**Dépolarisations accomplies :**
- [[Dépolarisation - Faire des erreurs et être extrêmement mauvais]] — peur de 10/10 → 0/10. Travail profond sur l'erreur inconsciente, la culpabilité, le droit à l'imperfection. *"Je suis en paix avec cette phase de ma vie."*
- [[Mini-Dépolarisation - Haute Énergie et performeur idéal]] — exploration des bénéfices de ne pas être le performeur idéal

**Ce qui ressurgit encore :**
Dans des conditions de budget épuisé — la réaction disproportionnée sur les spots qui "devraient être maîtrisés". Moins intense, moins durable qu'avant.

**Ce qui a changé :**
Conscientisation profonde que l'erreur n'est pas un verdict. La culpabilité sur les erreurs inconscientes est à 0/10 sur le fond. Ce qui ressurgit est résiduel et conditionné au budget corporel.

---

### Résultats-dépendance
**[documenté — Alexis 05-08-2026]**

**Prédiction centrale :**
*"Ma valeur comme joueur se mesure à mes résultats — la confiance monte et descend avec la variance."*

**État de conscientisation :**
🟢 Prédiction dépolarisée — immunité en cours de construction

**Conditions de résurgence (budget corporel) :**
- Mauvais run prolongé + mauvais sommeil
- Résultats financiers en baisse sur plusieurs semaines
- Isolement social (Siem Reap) combiné à mauvaise période

**Dépolarisations accomplies :**
- [[01 — Fondements théoriques/08 — Need for Cognition]] · [[01 — Fondements théoriques/10 — Agon · Alea · Mimicry · Ilinx]]
- [[L2 - Schémas Cognitifs/11 — Mobilisation compensatoire -- Suramplification]]
- [[L3 - Émotions/La Réactance]]
- [[Avantages d'avoir fait ma plus grosse session perdante au casino]] — travail direct sur la résultats-dépendance. Lâcher prise sur l'injonction aux résultats, standardisation des swings.
- [[Mini dépos frustré de run bad en live]] — bénéfices du bad run, construction d'une homéostasie sur la variance.
- [[Dépolarisation - revenir perdant du Séjour live]] — peur de 8/10 → 0/10. Travail sur le retour perdant et la spirale émotionnelle.
- [[Le sort s'acharne contre moi (USD-EURO)]] — travail sur le sentiment d'injustice lié aux résultats financiers.
- [[Mini dépolarisation - devenir un stratège de la performance]] — exploration de l'identité découplée des résultats.

**Ce qui ressurgit encore :**
En bad run prolongé combiné à mauvais sommeil. La Vision zoomée s'active : quelques sessions deviennent "le système est cassé". Moins systématique qu'avant, récupération plus rapide.

**Ce qui a changé :**
Capacité à lâcher prise sur le FOMO du casino. Meilleure récupération nocturne. Travail s'articule sur l'évolution, pas sur les résultats. Plusieurs dépos profondes accomplies — c'est l'une des zones les plus travaillées.

---

## IV. Tensions dynamiques

**Perfectionnisme protecteur ↔ Créativité comme identité**
Quand le Perfectionnisme est dominant (budget épuisé), tout play créatif non validé mathématiquement active la culpabilité. Quand le budget est plein, la créativité s'exprime librement. La tension se lit dans l'état du budget corporel.

**Résultats-dépendance ↔ Méta-accomplissement**
En état de budget plein : *"ma valeur = ma croissance."* En état de budget épuisé + bad run : *"ma valeur = mes résultats."* Le thermomètre de cette tension = la qualité du sommeil et la longueur du run.

**Intransigeance envers soi ↔ Honnêteté**
L'Intransigeance resurge spécifiquement sur les erreurs techniques en état épuisé. L'Honnêteté radicale opère dans les contextes de confiance. Ces deux prémisses s'activent rarement dans le même contexte.

---

## V. Cartographie des effets

| Schéma | État | L2 ciblé | L3 produit | Cascade |
|---|---|---|---|---|
| Perfectionnisme protecteur | 🟠 en cours | [[L2 - Schémas Cognitifs/12 — Orientation vers l'idéal -- Perfectionnisme\|Perfectionnisme]] · Piédestal · Pensée binaire | Frustration · Honte | Gap vision/compétences → Frustration cumulée → Honte |
| Intransigeance | 🟢 dépolarisé | Pensée binaire | Colère | Erreur → Colère → Étude compulsive |
| Résultats-dépendance | 🟢 dépolarisé | Vision zoomée · Sélect. mémorielle | Peur · Honte | Résultat → Peur → Retrait |
| Compétence bloquée (externe) | permanent | Confabulation | [[L3 - Émotions/La Réactance\|Réactance]] | Obstacle externe bloque la compétence → BAS frustré → Réactance [inféré depuis BIS/BAS + Brehm] |

---

## VI. Travail en cours

**Priorité :** Perfectionnisme protecteur — dépolarisation formelle à initier. La Haute Énergie dépo a commencé le travail, mais la peur du gâchis en haute énergie ressurgit encore.

**En progression :** Résultats-dépendance — immunité active, se construit par la pratique. Surveiller les conditions de budget épuisé + bad run comme déclencheur résiduel.

**Stabilisé :** Intransigeance — peur à 0/10 sur le fond (dépo Erreurs). Résiduel conditionné au budget corporel.

**LFB — levier pratique :**
Le budget corporel est le vrai point de levier pour ces trois schémas. Qualité du sommeil, gestion de la chaleur, récupération physique = protection contre la résurgence des prédictions défensives. C'est une intervention physiologique autant que psychologique.

---

## Sources

[^1]: Deci, E.L. & Ryan, R.M. (2000). *The "What" and "Why" of Goal Pursuits.* Psychological Inquiry, 11(4), 227–268.
[^2]: Rafaeli, E., Bernstein, D.P., & Young, J. (2018). *Schema Therapy.* Cognitive Behaviour Therapy, 47(4), 328–349.
[^3]: Crocker, J. & Park, L.E. (2004). *The Costly Pursuit of Self-Esteem.* Psychological Bulletin, 130(3), 392–414.
[^4]: Barrett, L.F. (2017). *How Emotions Are Made.* Houghton Mifflin. · Barrett, L.F. (2020). *Seven and a Half Lessons About the Brain.* Houghton Mifflin.

---

## Notes liées

- [[__Index]] · [[01 — Fondements théoriques/00 — Index]]
- [[05 — Évaluation du soi -- Pensée binaire]] · [[Évaluation du soi -- Pensée binaire]]
- [[04 — Calibration temporelle -- Vision zoomée]] · [[03 — Reconstruction mémorielle -- Sélectivité mémorielle]]
- [[01 — Résultat négatif → Peur → Retrait]] · [[03 — Erreur technique → Colère → Étude compulsive]]
- [[01 — Fondements théoriques/08 — Need for Cognition]] · [[01 — Fondements théoriques/10 — Agon · Alea · Mimicry · Ilinx]]
- [[L2 - Schémas Cognitifs/11 — Mobilisation compensatoire -- Suramplification]]
- [[L3 - Émotions/La Réactance]]
- [[Avantages d'avoir fait ma plus grosse session perdante au casino]]
- [[Dépolarisation - Faire des erreurs et être extrêmement mauvais]]
- [[Mini-Dépolarisation - Haute Énergie et performeur idéal]]
- [[Mini dépos frustré de run bad en live]]
- [[L2 - Schémas Cognitifs/12 — Orientation vers l'idéal -- Perfectionnisme|L2 Perfectionnisme / Ambition]]
